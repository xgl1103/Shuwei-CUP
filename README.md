...
> 前往 [Releases](../../releases) 页面，下载 `v1.0-data` 下的以下 **4 个文件**（缺一不可）：
>
> ```text
> B.z01
> B.z02
...
这是**分卷压缩包**（同一数据集切分为 4 卷）。请把 4 个文件下载到**同一个目录**，然后只需解压 `B.zip`，Bandizip / 7-Zip / WinRAR 会自动读取 `.z01`~`.z03` 分卷。

若用命令行：

```powershell
# Bandizip
& "C:\Program Files\Bandizip\bz.exe" x B.zip
# 或 7-Zip
7z x B.zip
```

解压后得到 `数据集/` 目录，请将其放入仓库根目录下的 `B题数据集/`：
...
之后即可按下方流程复现全部建模结果。各分卷的 SHA256 校验值见 Release 附件 `SHA256SUMS.txt`。
